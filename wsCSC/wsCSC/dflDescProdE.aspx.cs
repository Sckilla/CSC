...
public partial class dflDescProdE : System.Web.UI.Page
{
    //copia de la clase
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (res == "0")
            {
                Response.Write("<script language='javascript'>alert ('No se encontró el producto');document.location.href='dflCatalogoGralE.aspx'</script>");
            }
            else
            {
    ...
    protected void llenarDrop(int exist)
    {
        if (lblIdet.Text != "0")
        {
            ...
            {
                Response.Write("<script language='javascript'>alert('Ha ocurrido un error, intente de nuevo');document.location.href='dflCarritoE.aspx'</script>");
            }
        }
        dwlCant.Items.Clear();
