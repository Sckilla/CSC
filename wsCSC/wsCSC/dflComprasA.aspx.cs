...
public partial class dflComprasA : System.Web.UI.Page
{
    ...
    {
        if (objDatos.VerifExistVenCli(Application["cnn"].ToString(), int.Parse(Session["cveUsu"].ToString()), '1', 0) == "1") 
        {
            Response.Write("<script language='javascript'>alert('No se ha realizado ninguna compra o no existe ninguna compra activa para este usuario');document.location.href='dflMenuVentasA.aspx'</script>");
        }
        if (!IsPostBack)
        {
            LlenarGrid();
    ...
    protected void LlenarGrid()
    {
        ...
        dsVentas = objDatos.ListarVentasCli(Application["cnn"].ToString(), int.Parse(Session["cveUsu"].ToString()), '1', 0);
