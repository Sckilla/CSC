...
public partial class dflReporteProdCE : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (objDatos.VerifProdEX(Application["cnn"].ToString()) == "1")
            {
                Response.Write("<script language='javascript'>alert('No existen productos con existencias activos');document.location.href='dflMenuRep.aspx'</script>");
            }
            else
            {
                ...
                rptRep.Load(Server.MapPath("~/Reportes/crReporteProdCE.rpt"));//se declara la ruta de ubicación del reporte
