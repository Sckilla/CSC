...
public partial class dflSelecPagoC : System.Web.UI.Page
{
    ...
    void LlenarDrops()
    {
        ...
        ds = objDatos.ListarTarjetasCli(Application["cnn"].ToString(), int.Parse(Session["cveUsu"].ToString()), '0');
    ...
    protected void imgbContinuar_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            {
                string res = objDatos.AgregarTarjeta(Application["cnn"].ToString(), int.Parse(Session["cveUsu"].ToString()), '0', txtNumero.Text, txtCveTj.Text, dwlMes.SelectedValue.ToString(), dwlAño.SelectedValue.ToString(), dwlNTarjeta.SelectedValue.ToString());
                Session["MetP"] = 1;
                Session["IdP"] = objDatos.Id;
                if (res == "0")
                {                   
                    Response.Write("<script language='javascript'>alert ('Se ha registrado su nuevo método de pago');document.location.href='dflDetalleVentaC.aspx'</script>");
                }
                else if (res == "2")
                {
                    Response.Write("<script language='javascript'>alert ('Las modificaciones se han realizado correctamnete');document.location.href='dflDetalleVentaC.aspx'</script>");
                }
                else
                {
                    Response.Write("<script language='javascript'>document.location.href='dflDetalleVentaC.aspx'</script>");
                }
            }
            if (lblMet.Text == "Transferencia")
            {
                string res = objDatos.AgregarTransf(Application["cnn"].ToString(), int.Parse(Session["cveUsu"].ToString()), '0', txtNumero.Text, dwlDia.SelectedValue.ToString(), dwlMes.SelectedValue.ToString(), dwlAño.SelectedValue.ToString());
                if (res != "0")
                {
                    Session["MetP"] = 2;
                    Session["IdP"] = int.Parse(res);
                    Response.Write("<script language='javascript'>alert('Se ha recibido su método de pago para su verificación');document.location.href='dflDetalleVentaC.aspx'</script>");
                }
                else
                {
            ...
            {
                string res = objDatos.AgregarDeposito(Application["cnn"].ToString(), int.Parse(Session["cveUsu"].ToString()), '0', txtNumero.Text, dwlDia.SelectedValue.ToString(), dwlMes.SelectedValue.ToString(), dwlAño.SelectedValue.ToString());
                if (res != "0")
                {
                    Session["MetP"] = 3;
                    Session["IdP"] = int.Parse(res);
        ...
        {
            Response.Write("<script language='javascript'>alert('No se pudo realizar la acción');document.location.href='dflCarritoC.aspx'</script>");
        }
    }
