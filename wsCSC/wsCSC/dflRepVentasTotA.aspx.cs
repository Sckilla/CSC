...
public partial class dflRepVentasTotA : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if(objDatos.VerifExistVenCli(Application["cnn"].ToString(), int.Parse(Session["cveUsu"].ToString()), '1', 2) == "1")
            {
                Response.Write("<script language='javascript'>alert('No se ha realizado ninguna compra con este usuario');document.location.href='dflMenuVentasA.aspx'</script>");
            }
            else
            {
                ...
                ds = new DataSet();
                ds = objDatos.ReporteComprasT(Application["cnn"].ToString(), int.Parse(Session["cveUsu"].ToString()), '1');
