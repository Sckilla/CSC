...
public partial class dflRepVentasCanC : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (objDatos.VerifExistVenCli(Application["cnn"].ToString(), int.Parse(Session["cveUsu"].ToString()), '0', 1) == "1")
            {
                Response.Write("<script language='javascript'>alert('No se ha cancelado ninguna compra con este usuario');document.location.href='dflMenuVentasC.aspx'</script>");
            }
            else
            {
                ...
                ds = new DataSet();
                ds = objDatos.ReporteComprasC(Application["cnn"].ToString(), int.Parse(Session["cveUsu"].ToString()), '0');
