...
public partial class dflDescProd : System.Web.UI.Page
{
    //copia de la clase
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (res == "0")
            {
                Response.Write("<script language='javascript'>alert ('No se encontró el producto');document.location.href='dflCatalogoGralC.aspx'</script>");
            }
            else
            {
    ...
    protected void llenarDrop(int exist)
    {
        if(lblIdet.Text != "0")
        {
            ...
            {
                Response.Write("<script language='javascript'>alert('Ha ocurrido un error, intente de nuevo');document.location.href='dflCarritoC.aspx'</script>");
            }
        }
        dwlCant.Items.Clear();
    ...
    protected void imgbAddCart_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            if (lblIdet.Text != "0")
            {
                ...
                objDatos.AgregarDetCar(Application["cnn"].ToString(), '0', int.Parse(Session["cveUsu"].ToString()), int.Parse(Session["prodId"].ToString()), int.Parse(dwlCant.SelectedValue.ToString()));
                Response.Write("<script language='javascript'>alert('Se ha confirmado la nueva cantidad');document.location.href='dflCarritoC.aspx'</script>");
                return;
            }
            else
            {
