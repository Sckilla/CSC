...
public partial class dflReporteEmpReg : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (objDatos.VerifUsuReg(Application["cnn"].ToString(),1) == "1")
            {
                Response.Write("<script language='javascript'>alert('No existen empleados activos');document.location.href='dflMenuRep.aspx'</script>");
            }
            else
            {
                ...
                rptRep.SetParameterValue(0, 1);//Se asigna el valor del parámetro enviado a la base de datos
