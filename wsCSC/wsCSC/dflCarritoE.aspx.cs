...
public partial class dflCarritoE : System.Web.UI.Page
{
    ...
    protected void datosCarrito()
    {
        ...
        if (cantp == 0)
        {
            Response.Write("<script language='javascript'>alert('No se ha añadido ningún producto al carrito');document.location.href='dflInicioE.aspx'</script>");
        }
        ...
        double total = subtotal + (subtotal * imp) - (subtotal * desc);
    ...
    protected void LlenarGrid()
    {
        ...
        dsClientes = objDatos.ListarCarritoC(Application["cnn"].ToString(), '1', int.Parse(Session["cveUsu"].ToString()));
    ...
    {
        Response.Write("<script language='javascript'>document.location.href='dflSelecEnvE.aspx'</script>");
    }
    ...
    protected void imgbPagar_Click(object sender, ImageClickEventArgs e)
    {
        if (Session["MetE"].ToString() != "0")
        {
            ...
            Response.Write("<script language='javascript'>document.location.href='dflSelecPagoE.aspx'</script>");
        }
        else
        {
            Response.Write("<script language='javascript'>alert('No se ha seleccionado un método de envío');document.location.href='dflSelecEnvE.aspx'</script>");
        }
    }
    ...
    protected void gvCarrito_SelectedIndexChanging(object sender, GridViewSelectEventArgs e)
    {
        ...
        Response.Write("<script language='javascript'>alert('Agregue su nueva cantidad deseada');document.location.href='dflDescProdE.aspx'</script>");
    }
}