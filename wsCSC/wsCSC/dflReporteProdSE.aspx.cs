...
public partial class dflReporteProdSE : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (objDatos.VerifProdSEX(Application["cnn"].ToString()) == "1")
            {
                Response.Write("<script language='javascript'>alert('No existen productos sin existencias activos');document.location.href='dflMenuRep.aspx'</script>");
            }
            else
            {
                ...
                rptRep.Load(Server.MapPath("~/Reportes/crReporteProdSE.rpt"));//se declara la ruta de ubicación del reporte
